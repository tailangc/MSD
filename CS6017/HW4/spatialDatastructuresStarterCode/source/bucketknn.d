...
struct BucketKNN(size_t Dim){
    ...
    this(Point!Dim[] points, int nDivisions){
        ...
        foreach(const ref point; points){
            ...
            //distribute the points into their respective buckets based on their spatial coordinates. 
        }
    }

    Indices!Dim getIndices(Point!Dim p){
        //Computes the indices of the bucket containing point p
        Indices!Dim ret;
        foreach(i; 0..Dim){
    ...
    size_t getIndex(Indices!Dim ind){
        //Computes the linear index from multidimensional indices.
        size_t ret = 0;
        foreach(i, x; ind){
    ...
    Point!Dim[] rangeQuery(Point!Dim p, float r){
        //Finds all points within radius r of point p
