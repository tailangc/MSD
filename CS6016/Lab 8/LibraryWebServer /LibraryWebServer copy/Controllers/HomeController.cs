...
namespace LibraryWebServer.Controllers
{
    public class HomeController : Controller
    {
        ...
        public IActionResult CheckLogin( string name, int cardnum )
        {
            ...
            var matchingPatron = from patron in db.Patrons
                     where patron.Name == name && patron.CardNum == cardnum
                     select patron;

            if (matchingPatron.Any())
            {
        ...
        public ActionResult AllTitles()
        {
            ...
            into titleInventoryJoin
            from inventoryItem in titleInventoryJoin.DefaultIfEmpty()
            join checkedOut in db.CheckedOut on inventoryItem.Serial equals checkedOut.Serial into inventoryCheckedOutJoin
            from checkedOutItem in inventoryCheckedOutJoin.DefaultIfEmpty()
            join patron in db.Patrons on checkedOutItem.CardNum equals patron.CardNum into checkedOutPatronJoin
            from patronItem in checkedOutPatronJoin.DefaultIfEmpty()
            select new
            {
                isbn = title.Isbn,
                title = title.Title,
                author = title.Author,
                serial = inventoryItem == null ? null : (uint?)inventoryItem.Serial,
                name = patronItem == null ? "" : patronItem.Name
            };
...
return Json(query.ToArray());
        ...
        public ActionResult ListMyBooks()
        {
            var query = from title in db.Titles
                        ...
                        join checkedOut in db.CheckedOut on inventoryItem.Serial equals checkedOut.Serial into inventoryCheckedOutJoin
                        from checkedOutItem in inventoryCheckedOutJoin.DefaultIfEmpty()
                        where checkedOutItem.CardNum == card
                        select new
                        {
                            title = title.Title,
                            author = title.Author,
                            serial = checkedOutItem.Serial
                        };
            return Json(query.ToArray());
        }
