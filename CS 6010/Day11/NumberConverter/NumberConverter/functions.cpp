...
int StringToInt(std::string numStr, int base){
    ...
    int decNum = 0;
    if(numStr[0] == '-'){
        numStr = numStr.substr(1, numStr.size()-1);
        }
    for (int i = 0; i < numStr.size(); i++){
        numericValue = (numStr[i] - '0') * pow(base, (numStr.size()-i-1));
        decNum += numericValue;
    }
//    if (base == 10){
...
int ConvertDec (std::string deciStr){ //same logic for three functions
    int decNum = 0;
    int numericValue;
    if(deciStr[0] == '-'){
...
std::string intToHexadecimalString(int deciInt){
    ...
    for(int i = highExpo; i >= 0; i--){
        ...
        if(temp == 10){ // Use temp - 10 + 'a'for temp = 10~15 
            hexaStr += "A";
        }
        else if(temp == 11){
            hexaStr += "B";
